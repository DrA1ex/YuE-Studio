...
struct Song: Identifiable, Equatable {
    ...
    var engineLabel: String {
        switch engine { case "remote": return "iPhone"; case "mlx+remote": return "GPU, then iPhone"; case "ane": return "Neural Engine"; case "mlx": return "GPU (MLX)"; case "torch": return "GPU (PyTorch)"; case "mlx+ane": return "GPU, then Neural Engine"; default: return "" }
    }
        ...
        for run in runs {
            ...
            for dir in dirs where dir.lastPathComponent.hasPrefix("song") {
                ...
                songs.append(Song(run: run.lastPathComponent, index: Int(dir.lastPathComponent.dropFirst(4)) ?? 0, path: audio.path,
                                  ...
                                  title: metadata?["title"] as? String ?? result?["title"] as? String ?? "Song \(Int(dir.lastPathComponent.dropFirst(4)) ?? 0)",
