...
Whisper now prefers multilingual `large-v3-turbo` in FP32 on CPU in the isolated cover environment. Settings → Update / Repair Engine downloads the pinned new checkpoint. Complete legacy `small` snapshots remain usable offline with an explicit upgrade message; partial upgraded snapshots are skipped. Sequential long-form decoding replaces pipeline chunking: in the installed Transformers 4.45.2, the non-chunked path creates and forwards the actual attention mask for both short and long recordings. Word timestamps, three-beam decoding, temperature fallback, silence/log-probability thresholds and disabled previous-window conditioning are used. Eager attention is explicit because word alignment needs cross-attention weights. Only words with valid timestamps wholly inside near-digital silence are removed, retaining the original transcript. These settings do not establish transcription accuracy on singing.
...
Genre classification averages three bounded excerpts. CLAP adds one sufficiently separated descriptor per category: instrumentation, vocal delivery, mood, production and pace. Scores are cosine similarities, not confidence probabilities. Ambiguous categories are omitted. A missing CLAP installation leaves genre analysis available and displays an upgrade message. Existing users download the additional pinned model through Settings → Update / Repair Engine.
